...
here::i_am("eds223-labs/week10-template.Rmd")
```

Load Lidar data. 
```{r load-lidar}
# digital surface model (DSM)
dsm <- rast(here("./week10/SJER2013_DSM.tif"))

# digital terrain model (DTM)
dtm <- rast(here("./week10/SJER2013_DTM.tif"))
...
plot_centroids <- st_read(here("./week10/PlotCentroids/SJERPlotCentroids_Buffer.shp"))
...
veg_surveys <- read.csv((here("./week10/VegetationData/D17_2013_vegStr.csv"))) %>% 
  group_by(plotid) %>% 
  summarize('survey height' = max(stemheight, na.rm = TRUE))     # one output of max height for plot ids
